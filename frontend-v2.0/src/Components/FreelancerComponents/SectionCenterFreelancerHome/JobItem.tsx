    ...
    switch(size) {
      ...
      >
        ...
        >
          ...
          <p className="text-wrap text-truncate text-dark mb-2" style={{ fontSize: 16, margin: '8px 0', height: 38 }}>
            {data?.description?.length > 60 ? data?.description?.slice(0, 60) + '...' : data?.description}
          </p>
          <Space size={'small'} wrap style={{ minHeight: '80px' }}>
            {data?.categories
              ?.filter(c => !!c)
              ?.map((c, ix) => (
                <Link
        ...
        <div style={{ fontSize: '0.9em' }} className="my-lg-1 fw-bold me-1 text-muted">
      ...
      >
        ...
        >
          ...
          <p className="text-wrap text-truncate text-dark mb-2" style={{ fontSize: 14, margin: '8px 0', minHeight: 12, maxHeight: 40 }}>
            {data?.description?.length > 42 ? data?.description?.slice(0, 42) + '...' : data?.description}
          </p>
          <Space size={'small'}  wrap style={{ minHeight: '40px', maxHeight: 40 }}>
            {data?.categories
              ?.filter(c => !!c)
              ?.map((c, ix) => (
                <Link
