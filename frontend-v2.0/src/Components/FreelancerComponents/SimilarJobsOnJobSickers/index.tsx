...
export default function SimilarJobsOnJobSickers({ id }: any) {
  const size = useWindowSize()
  const { t } = useTranslation(['main']);
  const [similarJobs, setSimilarJobs] = React.useState<any>([])

  useEffect(() => {
    getSimilarJobs(id, { limit: 8 }).then((res) => {
  ...
  return (
    <div className="mb-4">
      <div className="bg-white py-lg-1 px-4 mt-4  border  row py-xs-5">
        <h4 className="fw-bold py-3">{t("Similar Jobs on JobSickers")}</h4>
      </div>
      {
        size > 1000 ? <Row justify="center" className="mt-3">
          <Col span={23}>
            <Carousel arrows slidesToShow={3} {...settings} style={{ width: '100%' }}>
