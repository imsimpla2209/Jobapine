...
import { fakeJobsState, fakeOfferData } from 'Store/fake-state';

export default function OfferCard({ clientID, jobID, getOffers }) {

	const offer = fakeOfferData
		...
		<div className="col-11 mx-auto bg-gray border border-gray rounded p-5 mb-4 text-center">
			{
				...
				<>
					...
					<p><strong>Contract Budget: </strong>${offer?.jobBudget}</p>
					<p><strong>Contract Payment Type: </strong>{offer?.jobPaymentType}</p>
					<p><strong>Contract Due Date: </strong>{offer?.dueDate}</p>
					<button
