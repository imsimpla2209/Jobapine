          ...
          <div className="col-lg-9 pt-lg-2">
            <Link
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                navigate(`/job/${item.jobID}`, {state: `${item.jobID}`})
              }}
            ...
            <div className="btn-group float-sm-end">
              ...
              >
                <i
                  ...
                  className={`${user.savedJobs?.includes(item.jobID) ? 'fas fa-heart text-jobsicker' : 'far fa-heart'}`
                  } aria-hidden="true" />
        ...
        <p style={{ fontSize: "0.9em" }}>
          <span className="text-muted">
            <span className="fw-bold" id="contract-type">
              {lang === 'vi' ? item?.jobPaymentTypeAr : item?.jobPaymentType}
            </span>
            <span> - </span>
            <span id="experience-level">{lang === 'vi' ? item?.jobExperienceLevelAr : item?.jobExperienceLevel}</span>
            <span> - </span>
