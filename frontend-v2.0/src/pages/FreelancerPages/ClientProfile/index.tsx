...
export default function ClientProfile() {
  ...
  useEffect(() => {
    if (inView && clientData) {
      console.log('inView', inView, ref)
      setStart(performance.now())
    }
    return () => {
      console.log('out', (performance.now() - start) / 1000)
      const extraStart = isRcmd ? start - 100000 : start
      const event = ETrackingEvent.VIEWCLIENT
      clientData?.jobs?.map(j => updateTrackingData(ETrackingType.JOBS, j, event, '', extraStart))
      clientData?.preferJobType?.map(c =>
        updateTrackingData(ETrackingType.CATEGORIES, c?.id, event, c?.name, extraStart)
      )
      clientData?.findingSkills?.map(s =>
        updateTrackingData(
      ...
      {jobDatas !== null ? (
        <div className="container-md container-fluid-sm my-lg-5 my-sm-4 py-xs-5 px-md-5">
              ...
              <div className="col-lg-3 col-xs-3">
                <Space size="middle" direction="vertical" wrap className="mb-2 bg-white p-4">
                  ...
                  <p>
                    ...
                    {clientData?.user?.isEmailVerified ? `${t('Verified')}✅` : `${t('Not Verified')} ⛔`}{' '}
                  </p>
                  <p>
                    <strong>{t('Phone Verification')}: </strong>{' '}
                    {clientData?.user?.isPhoneVerified ? `${t('Verified')}✅` : `${t('Not Verified')} ⛔`}
                  </p>

                  <h5 className="text-muted mt-3" style={{ lineHeight: 0 }}>
                    {t('Rating from Freelancers')}
                  </h5>
          ...
          <div className="row  me-md-1">
            <div className="col-lg-12 col-xs-12"></div>
          </div>
        </div>
      ) : (
