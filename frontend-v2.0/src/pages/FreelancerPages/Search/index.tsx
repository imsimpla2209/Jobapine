...
export default function Search() {
	...
	const handleDurationRange = useCallback((c: any) => {
		console.log('time', c)
	...
	const handlePayAmount = useCallback((c: any = 0, d: any = 1000000) => {
	...
	const handleProposalsAmount = useCallback((c: any) => {
	...
	const handleBudgetAmount = useCallback((c: any = 0, d: any = 1000000) => {
	...
	const onNoEmployeeChange = useCallback((c: any) => {
	...
	const onSkillsChange = useCallback((c: any) => {
	...
	const onCatsChange = useCallback((c: any) => {
	...
	const onLocationChange = useCallback((l: any) => {
	...
	const onStatusChange = useCallback((l: any) => {
	...
	const onPayTypeChange = useCallback((l: any) => {
					...
					<div>
						...
						<div className="row" style={{ borderTop: '1px solid #ccc', paddingTop: 24 }}>
							<div className="col-11">
								<SearchBarJobsFreelancer showPath={false} useIndexSearch={false} textSearch={text} setSearchText={searchFilterText} />
							</div>
							<Button onClick={() => handleSearh()} className="col-1" type="primary" shape="circle" icon={<SearchOutlined />} />
						</div>
						<div style={{ borderBottom: '2px solid #ccc', borderRadius: 8, paddingLeft: 8, paddingRight: 8, paddingTop: 8, paddingBottom: 8, width: '100%' }}>
							{
								!isEmpty(filterOption) && <Space wrap>
									{
										filterOption?.amount &&
										<FilterTag color="#ff8c00">
											{filterOption.amount}
										</FilterTag>
									}{
										filterOption?.budget &&
										...
										filterOption?.duration &&
										<FilterTag color="#3fa182">
											{filterOption.duration}
										</FilterTag>
									}{
										filterOption?.nOEmployee &&
										<FilterTag color="#0e569e">
											{filterOption.nOEmployee}
										</FilterTag>
									}{
										filterOption?.skills &&
										...
										filterOption?.proposals &&
										<FilterTag color="#ab0c56">
											{filterOption?.proposals}
										</FilterTag>
									}{
									...
									<Button type="text" onClick={removeFilterOptions} style={{ color: '#6600cc', fontSize: 17, padding: 2 }}>{t("Clear Filter")}</Button>
								</Space>
							}
						</div>
					</div>
